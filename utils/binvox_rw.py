...
def write(voxel_model, fp):
    ...
    s='dim '+' '.join(map(str, voxel_model.dims))+'\n'
    fp.write(s.encode(encoding='utf-8'))
    
    s='translate '+' '.join(map(str, voxel_model.translate))+'\n'
    fp.write(s.encode(encoding='utf-8'))
    
    s='scale '+str(voxel_model.scale)+'\n'
    fp.write(s.encode(encoding='utf-8'))
    ...
    for c in voxels_flat:
        if c==state:
            ...
            if ctr==255:
                fp.write(chr(state).encode('latin1'))
                fp.write(chr(ctr).encode('latin1'))
                ctr = 0
        else:
